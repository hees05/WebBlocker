...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  let hasSaidExpectedPhrase = false;
  let hasGivenReflection = false;
  ...
  try {
    ...
    const speech = `Access denied. You tried to access a page with the word "${keyword}". Please repeat after me: I will not search up ${keyword} again. Then explain in 10 words why you tried.`;      
  ...
  const expectedPhraseTwo = 'I\'m sorry';
  ...
  recognition.onresult = (event) => {
      ...
      // Unlocks go back ONLY after expected phrase AND reflection
      if (!hasSaidExpectedPhrase && transcript.includes(expectedPhrase)) {
        hasSaidExpectedPhrase = true;
        hasGivenReflection = true;
        goBackButton.disabled = false;
        liveMsg.textContent = "✅ Phrase accepted. Now you owe me an apology.";
        recognition.start(); // keep listening
        return;
      }
      
      /*
      if (hasSaidExpectedPhrase && !hasGivenReflection) {
        if (transcript.includes("apologise")) {
          hasGivenReflection = true;
          goBackButton.disabled = false;
          liveMsg.textContent = "✅ Reflection recorded. You may now go back.";
          showTemporaryBanner("Thank you for reflecting.", "success");
        } else {
          liveMsg.textContent = "Please include 'I'm sorry' in your reflection.";
          recognition.start(); // Keep listening
        }
        return;
      }
  ...
  // This disables the go back button until confirmed by the Neuphonic package.
  if (goBackButton) {
